/*Writete a C program to compare the cost of tiling three floors and determine the most expensive one using functions and structure. (else-if ladder)
...
void compare_costs(Floor f1, Floor f2, Floor f3) {
    ...
    else if (f3.total_cost > f1.total_cost && f3.total_cost > f2.total_cost) {
        printf("\nMost expensive: %s\n", f3.name);
    }
    else {
        printf("\nSome floors have equal cost.\n");
    }
}